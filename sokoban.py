...
class Menu(object):
    ...
    def load_levels(self, filenames=None): 
        if not filenames:            
            filenames = tuple(map(os.path.abspath, [os.path.join(_LEVEL_FOLDER, x) for x in os.listdir(_LEVEL_FOLDER)]))

        self.app.grid_forget()                                                  
        level_files = sorted(filenames)
...
class Application(tk.Frame):
    ...
    def start_next_level(self):
        ...
        if len(self.level_files) > 0:
            self.current_level = self.level_files.pop(0)
            level = open(self.current_level, "r")
            self.grid()
            self.load_level(level)
            self.master.title(self.current_level.split("/")[-1])
        else:
            self.current_level = None
            CompleteDialog()
    ...
    def restart_level(self):
        if self.current_level:
            self.level_files.insert(0, self.current_level)
